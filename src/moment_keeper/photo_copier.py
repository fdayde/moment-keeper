...
class PhotoCopier:
    ...
    def _transferer(
        ...
        """Transfère un fichier vers un dossier de destination via l'opération donnée.

        Args:
            source: Chemin du fichier source
            destination_dir: Dossier de destination
            operation: Fonction shutil (move ou copy2) à exécuter

        Returns:
            Path du fichier transféré
    ...
    def deplacer_fichier(self, source: Path, destination_dir: Path) -> Path:
        ...
        Raises:
            ...
            ValueError: Si le chemin de destination est invalide (path traversal)
        """
        return self._transferer(source, destination_dir, shutil.move)
    ...
    def copier_fichier(self, source: Path, destination_dir: Path) -> Path:
        ...
        Raises:
            ...
            ValueError: Si le chemin de destination est invalide (path traversal)
        """
        return self._transferer(source, destination_dir, shutil.copy2)
